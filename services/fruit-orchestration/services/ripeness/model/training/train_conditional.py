...
from data.data_multitask import make_loaders
...
def train_phase(model, ltr, lva, device, epochs, lr, wd, ckpt_dir, tag, ce, patience=2):
    ...
    try:
        for ep in range(1, epochs + 1):
            ...
            acc, f1 = evaluate(model, lva, device)
            sch.step()
            print(f"[{tag} Epoch {ep}] val_acc={acc:.3f} val_f1={f1:.3f}")

            if f1 > best_f1 + 1e-4:
                best_f1 = f1
                best_state = {k: v.cpu() for k, v in model.state_dict().items()}
                torch.save(best_state, os.path.join(ckpt_dir, f"best_conditional_{tag}.pt"))
                no_improve = 0
            else:
                no_improve += 1
        ...
        print("KeyboardInterrupt — saving best checkpoint so far...")
...
if __name__ == "__main__":
    ...
    print("Saved:", os.path.join(cfg["checkpoint_dir"], "best_conditional.pt"), "| best F1:", best_f1)
