class NotificationManager {
      ...
      <div class="notification-item">
        <div class="notification-header">
          ...
          <div class="notification-actions">
            <button class="btn btn-edit" data-schedule-id="${schedule.schedule_id}">Edit</button>
            <button class="btn btn-delete" data-schedule-id="${schedule.schedule_id}">Delete</button>
          </div>
        </div>
  ...
  attachNotificationListeners() {
    // Edit buttons
    const editButtons = this.notificationsList.querySelectorAll('.btn-edit')
    editButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        const scheduleId = e.target.getAttribute('data-schedule-id')
        this.editNotification(scheduleId)
      })
    })
    
    // Delete buttons
    const deleteButtons = this.notificationsList.querySelectorAll('.btn-delete')
    deleteButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        const scheduleId = e.target.getAttribute('data-schedule-id')
        this.deleteNotification(scheduleId)
      })
    })
  }
  ...
  async editNotification(scheduleId) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Change tab button text to "Edit Notification"
  ...
  resetForm() {
    ...
    // Reset the tab button text back to "Add New"
    const addTabBtn = document.querySelector('button[data-tab="add"]')
    if (addTabBtn) {
      addTabBtn.textContent = "Add New"
    }
