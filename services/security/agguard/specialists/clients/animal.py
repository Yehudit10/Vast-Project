...
StubClass = getattr(pbrpc, "ClassifierServiceStub",
                    getattr(pbrpc, "ClassifierStub", None))
...
class GrpcAnimalClassifierClient:
    def __init__(self, address: str, model_name: str = "yolo-cls",
        ...
        log.info("GrpcAnimalClassifierClient -> %s (model=%s)",
                 self.address, self.model_name)

    # ───────────────────────────────────────────────
    # Padded crop extractor (20% padding)
    # ───────────────────────────────────────────────
    @staticmethod
    def _safe_crop(frame_bgr: np.ndarray, box: Box,
                   pad_ratio: float = 0.2) -> Optional[np.ndarray]:
        """
        Return the crop with padding around the box (20% by default).
    ...
    def classify(self, frame_bgr: np.ndarray, boxes: List[Box]):
        ...
        for b in boxes:
            # Use padded crop
            crop = self._safe_crop(frame_bgr, b, pad_ratio=0.2)
            if crop is None:
                continue
            ...
            # IMPORTANT: send original box (not padded box)
            crops.append(pb.Crop(
                jpeg=buf.tobytes(),
