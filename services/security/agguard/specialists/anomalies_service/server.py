...
class ClipClassifierServicer(ServicerBase):
    ...
    def __init__(self):
        ...
        # --- Load CLIP model ---
        log.info("[CLIPClassifier] 🔹 Loading OpenCLIP model %s/%s ...", self.model_name, self.pretrained)
        t0 = time.time()
        ...
        MODEL_LOAD_SEC.labels(service=service_name).set(load_time)
        log.info("[CLIPClassifier] ✅ Model loaded in %.1fs on %s", load_time, self.device)
    ...
    def _classify_one(self, image_emb: torch.Tensor, subject: str) -> float:
        ...
        with torch.inference_mode():
            ...
            prob_climbing = float(logits[0, 0].item())
    ...
    def Classify(self, request: pb.ClassifyRequest, context) -> pb.ClassifyResponse:
        INFER_REQUESTS.labels(service=self.service_name).inc()
        t0 = time.time()
        ...
        try:
            ...
            for c in request.crops:
                try:
                    ...
                    log.warning("[CLIPClassifier] Failed to decode crop: %s", e)
            ...
            dt = time.time() - t0
            INFER_LATENCY.labels(service=self.service_name).observe(dt)
            log.info("[CLIPClassifier] → %d predictions in %.2fs", len(resp.preds), dt)
            return resp

        except Exception as e:
            INFER_ERRORS.labels(service=self.service_name).inc()
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(str(e))
            log.exception("[CLIPClassifier] Inference failed: %s", e)
...
def serve():
    ...
    start_metrics_server()
    log.info("[CLIPClassifier] 📊 Prometheus metrics exposed on :%d/metrics", metrics_port)

    server = grpc.server(futures.ThreadPoolExecutor(max_workers=2))
    add_servicer(ClipClassifierServicer(), server)
    server.add_insecure_port(f"[::]:{port}")
    log.info("[CLIPClassifier] 🚀 gRPC server listening on :%d", port)
    server.start()
