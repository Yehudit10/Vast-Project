...
class ClassDispatch:
    ...
    def run(self, frame_bgr, dets: List[Detection]) -> Dict[str, object]:
        ...
        # ─────────────────────────────────────────────
        # 🧩 CONDITIONAL SECOND-STAGE CALL
        # ─────────────────────────────────────────────
        if "animal" in outputs and outputs["animal"]:
            intruding_preds = [
                p for p in outputs["animal"]
                if isinstance(p, dict) and p.get("label") != "other"
            ]
            if intruding_preds:
                subjects = [p["label"] for p in intruding_preds]
                log.info("[ClassDispatch] Intruding animal(s): %s → checking climbing_fence", subjects)

                # 🧠 Call CLIP classifier only if configured
                if "intruding animal" in self._by_class:
                    for fn in self._by_class["intruding animal"]:
                        try:
                            preds = fn(frame_bgr, [p["box"] for p in intruding_preds], subjects)
                            if preds:
                                # Store CLIP results under 'climbing_fence'
                                outputs["climbing_fence"] = preds
                                # Also propagate subjects so aggregator knows which animal climbed
                                outputs["_subject"] = subjects
                                log.info("[ClassDispatch] climbing_fence predictions: %s", preds)
                        except Exception as e:
                            log.exception("intruding-animal specialist failed: %s", e)
                else:
                    log.debug("[ClassDispatch] No 'intruding animal' specialist configured.")
