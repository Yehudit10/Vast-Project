...
def segment_waveform(
    ...
    """
    Splits waveform into overlapping fixed-size windows.
    Returns list of 1D numpy arrays (segments), each of length window_sec * sr.
    """
    ...
    i = 0
    while i + win <= n:
        seg = wav[i: i + win].astype(np.float32)
        segments.append(seg)
        i += hop
