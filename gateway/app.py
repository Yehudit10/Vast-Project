...
#     # ---- Demo Prometheus metrics for Grafana ----
#     registry = CollectorRegistry()
#     g_status = Gauge("sensor_status", "1=active, 0=inactive", ["sensor"], registry=registry)
#     g_active = Gauge("sensors_active_total", "Total active sensors", registry=registry)
#     g_last  = Gauge("sensor_last_seen_timestamp_seconds", "Last seen unix ts", ["sensor"], registry=registry)

#     SENSOR_IDS = [f"sensor_{i:02d}" for i in range(30)]

#     def _update_demo_metrics():
#         now = int(time.time())
#         # Toggle pattern changes every 30 seconds
#         phase = (now // 30) % 5
#         active = 0
#         rng = random.Random(phase)
#         for sid in SENSOR_IDS:
#             # pseudo status: mostly 1, some 0 depending on phase
#             is_up = 1 if rng.random() > 0.2 else 0
#             g_status.labels(sensor=sid).set(is_up)
#             if is_up:
#                 active += 1
#                 g_last.labels(sensor=sid).set(now - rng.randint(0, 20))
#             else:
#                 g_last.labels(sensor=sid).set(now - rng.randint(60, 300))
#         g_active.set(active)

#     @app.get("/metrics")
#     def metrics() -> Response:
#         _update_demo_metrics()
