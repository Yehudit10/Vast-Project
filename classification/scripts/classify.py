...
# Allow running both from repo root ("python -m classification.scripts.classify")
# and from inside the classification/ folder ("python -m scripts.classify").
import sys, pathlib
...
def main() -> None:
    ...
    try:
        if args.write_db:
            ...
            # importing here to keep import paths simple
            ...
            upsert_run(conn, dict(
                ...
                agg=args.agg,
                topk=1,  # keep >=1 to satisfy DB CHECK constraint
                device=args.device,
                code_version=getenv("GIT_COMMIT", ""),
                notes=f"head_type=unknown; unknown_threshold={args.unknown_threshold}"
            ))
        ...
        for f in files:
            try:
                ...
                # Aggregated AudioSet (diagnostic)
                ...
                if head is not None and any(hp is not None for hp in per_window_head):
                    ...
                    LOGGER.info("final label=%s (top1_prob=%.4f, threshold=%.3f)", pred_label, (pred_prob or -1.0), args.unknown_threshold)
                ...
                if conn is not None:
                    ...
                    processing_ms = (time.perf_counter() - t_file_start) * 1000.0

                    agg_row = dict(
                        run_id=run_id,
                        file_id=file_id,
                        audioset_topk_json={"topk": []},  # kept minimal; you can fill if needed
        ...
        LOGGER.info("done processing %d files", len(files))
