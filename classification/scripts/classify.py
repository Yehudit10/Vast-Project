# CNN14 + RandomForest head, no AudioSet top-k prints, with "another" fallback.
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Production-ready classifier: CNN14 backbone + RF head.")
    ...
    ap.add_argument("--head", default=os.getenv("HEAD", "models/head_cnn14_rf.joblib"))
    ...
    # unknown fallback (default = 0.25 per your decision)
    ap.add_argument("--unknown-threshold", type=float, default=float(os.getenv("UNKNOWN_THRESHOLD", 0.25)))
    ...
    if root.is_file() and (root.suffix.lower() not in SUPPORTED_EXTS):
        LOGGER.error("unsupported extension: %s", root.suffix)
        sys.exit(4)
    ...
    try:
        ...
        LOGGER.exception("failed to load CNN14 model: %s", e)
        sys.exit(2)
    ...
    if args.head:
        ...
        if not hp.exists():
            LOGGER.error("head not found at %s", hp.resolve())
            sys.exit(6)
        try:
            import joblib
            ...
            if meta_path.exists():
                ...
                if "class_order" in head_meta:
                    KNOWN_HEAD_CLASSES = [str(x).strip() for x in head_meta["class_order"] if str(x).strip() != "another"]
            ...
            LOGGER.exception("failed to load head '%s': %s", args.head, e)
            sys.exit(7)
    ...
    try:
        if args.write_db:
            if not args.db_url:
                ...
                sys.exit(3)
            from classification.core.db_io_pg import open_db, upsert_run
            ...
            upsert_run(conn, dict(
                ...
                notes=f"head_type=rf; unknown_threshold={args.unknown_threshold}",
            ))
        ...
        for f in files:
            try:
                ...
                for (_t0, _t1, seg) in windows:
                    probs, labels, emb = run_inference_with_embedding(at, seg)
                    if emb is None:
                        emb = run_cnn14_embedding(at, seg)
                    emb = np.asarray(emb, dtype=np.float32).reshape(-1)

                    if (head_expected_dim is not None) and (emb.size != head_expected_dim):
                        per_window_head.append(None)
                        continue

                    try:
                        hp = head.predict_proba(emb.reshape(1, -1))[0]  # type: ignore[attr-defined]
                    except Exception:
                        hp = None
                ...
                if valid_hps:
                    ...
                    LOGGER.info("%s -> %s (p=%.3f)", f.name, pred_label, (pred_prob or float("nan")))
                else:
                    LOGGER.warning("no valid head predictions for %s", f.name)

                if conn is not None:
                    from classification.core.db_io_pg import upsert_file, upsert_file_aggregate
                    file_id = upsert_file(conn, str(f), duration_s, SAMPLE_RATE, size_bytes)
                    upsert_file_aggregate(conn, dict(
                        run_id=run_id,
                        file_id=file_id,
                        audioset_topk_json=None,
        ...
        LOGGER.info("done %d files in %.2fs", len(files), time.perf_counter() - t0)

    finally:
        if conn is not None:
            try:
                from classification.core.db_io_pg import finish_run
