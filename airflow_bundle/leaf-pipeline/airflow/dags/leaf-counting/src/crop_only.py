...
def _load_jsons(inp: Path):
    ...
    if not jdir.exists():
        raise SystemExit(f"[ERR] Expected JSON dir not found: {jdir} (run detect_only.py first)")
    # חיפוש רקורסיבי בכל תתי-התיקיות
...
#              minio_prefix: str="crops", minio_secure: bool=False):
...
#         else:
#             if orig_dir is None:
#                 raise SystemExit("[ERR] JSON חסר source_path; ספקי --orig לתיקיית המקור")
...
#         # מבנה פלט: תת תיקייה לכל תמונה (אלא אם --flat)
#         if flat:
#             dest_dir = ensure_dir(out_dir)
#             minio_subprefix = minio_prefix
#         else:
#             dest_dir = ensure_dir(out_dir / Path(j["image"]).stem)
#             minio_subprefix = f"{minio_prefix}/{Path(j['image']).stem}"
...
def run_crop(inp: Path, out_dir: Path, size: int=224, margin: float=0.1, min_wh: int=8,
    ...
    count = 0
    for jp, j in _load_jsons(inp):
        # עדיפות: source_path; אם אין – rel_path; ואם אין – נדרש --orig
        if "source_path" in j:
        ...
        # תיקיית יעד: out/<rel_parent>/<rel_stem>/...
        if flat:
            dest_dir = ensure_dir(out_dir)
            minio_subprefix = minio_prefix
        else:
