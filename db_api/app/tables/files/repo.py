...
def _ensure_json_text(obj: Any) -> Optional[str]:
    """
    אם obj הוא dict/list – נמיר למחרוזת JSON.
    אם None – נחזיר None.
    אם כבר מחרוזת – נחזיר כמות שהיא.
    """
    if obj is None:
        return None
    if isinstance(obj, (dict, list)):
        return json.dumps(obj, ensure_ascii=False)
    return obj  # כבר מחרוזת/טיפוס מתאים
...
def upsert_file(payload: Dict[str, Any]) -> None:
    ...
    # עותק מקומי + המרות נדרשות
    payload = dict(payload)
    payload["metadata"] = _ensure_json_text(payload.get("metadata"))

    # לוודא שתמיד יש מפתח footprint; ריק -> None
