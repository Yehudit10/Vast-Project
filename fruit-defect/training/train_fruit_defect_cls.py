...
def label_from_path(p: Path) -> int:
    """
    בינארי: 0 = healthy/fresh, 1 = defect/rotten/… (לפי שם התיקייה/נתיב)
    מתאים למבנה עם 28 תיקיות כמו Apple_Healthy / Apple_Rotten וכו’.
    """
    ...
    if any(k in name for k in ["rotten", "defect", "bad", "decay", "mold", "mould", "damaged", "spoiled"]):
        return 1
    # ברירת מחדל זהירה: ננסה לפי שם התיקייה העליונה
...
def build_model(backbone: str):
    """
    תומך ב: mobilenet_v3_small (ברירת מחדל מהיר) או resnet18 (מדויק יותר אך כבד יותר).
    משתמש ב-weights מאומנים מראש על ImageNet אם זמינים.
    """
